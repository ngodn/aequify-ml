"""
Rolling Low Kernel - GPU-accelerated rolling minimum price computation.

This kernel computes the rolling minimum price over a fixed window of trades.
Used by grid_search_short to detect price rises from recent lows.

Architecture:
    1. Simple per-thread kernel (each thread handles one output index)
    2. Scans the window range to find minimum price
    3. For indices < window: output is NO_LOW_SENTINEL (invalid/no data)

Formula: rolling_low[i] = min(prices[i-window:i])

This is a direct port of the Python/CUDA kernel from apex_29_ultra.py:
    @cuda.jit
    def rolling_low_kernel(prices, window, out, n):
        i = cuda.grid(1)
        if i >= n:
            return
        if i < window:
...
Usage:
    rolling_low_gpu(ctx, prices_dev, result_dev, window, n)
"""
...
from gpu.primitives.warp import lane_group_min
...
# Sentinel value for indices < window (matches Python implementation)
...
@always_inline
fn warp_reduce_min_f64(val: Float64) -> Float64:
    """Warp-level min reduction using lane_group_min.

    Uses Float32 internally for compatibility with lane_group_min,
    then converts back to Float64.
    """
    var val_f32 = Float32(val)
    var result = lane_group_min[num_lanes=WARP_SIZE](val_f32)
...
fn _rolling_low_simple_kernel(
    ...
    Args:
        prices: Trade prices.
        result: Output rolling low array.
    ...
    # For indices < window, output is sentinel value (no valid data)
    if i < Int(window):
        result[i] = NO_LOW_SENTINEL
        return

    # Find min in window [i-window, i)
    var min_val = prices[i - Int(window)]
...
fn _rolling_low_warp_kernel(
    prices: UnsafePointer[Float64, MutAnyOrigin],
    result: UnsafePointer[Float64, MutAnyOrigin],
    window: Int32,
    n: Int32,
):
    """Warp-cooperative rolling low kernel.
    ...
    # Use positive infinity as identity for min reduction
    var local_min: Float64 = 1e308
    ...
    # Warp-level reduction to find minimum across all lanes
    var global_min = warp_reduce_min_f64(local_min)
...
fn rolling_low_gpu(
    ...
    """Run rolling low computation on GPU using warp-cooperative scanning.
    ...
    Args:
        ...
        result_dev: Device buffer for output rolling low.
        window: Rolling window size (number of trades).
        n: Number of trades.
    """
    ...
    ctx.enqueue_function_checked[_rolling_low_warp_kernel, _rolling_low_warp_kernel](
...
fn rolling_low_gpu_simple(
    ...
    """Run rolling low computation on GPU using simple per-thread kernel.
    ...
    ctx.enqueue_function_checked[_rolling_low_simple_kernel, _rolling_low_simple_kernel](
        prices_dev.unsafe_ptr(),
        result_dev.unsafe_ptr(),
        Int32(window),
        Int32(n),
...
fn _rolling_low_multi_kernel(
    ...
    """Multi-window rolling low kernel.

    Computes rolling lows for all windows in a single kernel launch.
    Output layout: result[w_idx * n + i] = rolling_low for window w_idx at index i
    ...
    for w_idx in range(Int(n_windows)):
        ...
        if i < window:
            result[out_idx] = NO_LOW_SENTINEL
        else:
            var min_val = prices[i - window]
            for j in range(i - window + 1, i):
                var p = prices[j]
                if p < min_val:
                    min_val = p
...
fn rolling_low_multi_gpu(
    ...
    """Compute rolling lows for multiple windows in single kernel launch.
    ...
    ctx.enqueue_function_checked[_rolling_low_multi_kernel, _rolling_low_multi_kernel](
        prices_dev.unsafe_ptr(),
        result_dev.unsafe_ptr(),
...
fn rolling_low_cpu[
    ...
    """CPU reference implementation of rolling low.
...
fn rolling_low_cpu_parallel(
    ...
    """CPU parallel implementation of rolling low.

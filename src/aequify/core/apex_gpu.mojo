"""
Aequify APEX GPU - GPU context and Python bindings.

Provides GPUContext struct for Mojo and Python-callable functions.
Named apex_gpu to avoid shadowing the system gpu module.
"""
...
fn _get_logger() raises -> PythonObject:
    ...
    return logging_mod.get_logger("aequify.core.apex_gpu")
...
@export
fn gpu_available_py() raises -> PythonObject:
    var logger = _get_logger()
...
@export
fn has_nvidia_gpu_py() raises -> PythonObject:
    var logger = _get_logger()
...
@export
fn has_amd_gpu_py() raises -> PythonObject:
    var logger = _get_logger()
...
@export
fn has_apple_gpu_py() raises -> PythonObject:
    var logger = _get_logger()
...
@export
fn gpu_vendor_py() raises -> PythonObject:
    var logger = _get_logger()
...
@export
fn device_count_py() raises -> PythonObject:
    var logger = _get_logger()
...
@export
fn get_all_info_py(device_id: PythonObject) raises -> PythonObject:
    var logger = _get_logger()
    var dev_id = Int(device_id) if device_id else 0
    logger.debug("get_all_info() called for device " + String(dev_id))
    var gpu = GPUContext(dev_id)
    return gpu.to_dict()


@export
fn get_all_info_py_no_args() raises -> PythonObject:
    var logger = _get_logger()
    logger.debug("get_all_info() called for device 0 (default)")
    var gpu = GPUContext(0)
    return gpu.to_dict()


@export
fn PyInit_apex_gpu() -> PythonObject:
    """Initialize the apex_gpu Python module."""
    try:
        var m = PythonModuleBuilder("apex_gpu")
        ...
        # Export no-args version as default (device 0)
        m.def_function[get_all_info_py_no_args]("get_all_info", docstring="Get all GPU info as dict (device 0)")
        m.def_function[get_all_info_py]("get_all_info_device", docstring="Get GPU info for specific device")
        return m.finalize()
    except e:
        return PythonObject()

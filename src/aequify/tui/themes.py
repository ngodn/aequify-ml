"""
Aequify TUI Theming System.

Provides theme models, built-in themes, and utilities for customizing
the TUI appearance.
"""
...
class Theme(BaseModel):
    ...
    def to_textual_theme(self) -> TextualTheme:
        ...
        if self.text_area:
            ...
            if self.text_area.cursor_line_gutter:
                variables["text-area-cursor-line-gutter"] = self.text_area.cursor_line_gutter
    ...
    def text_area_theme_from_theme_variables(
        ...
        syntax_styles = {
            "string": Style.parse(variables.get("syntax-json-string", variables["text-accent"])),
            "number": Style.parse(variables.get("syntax-json-number", variables["text-secondary"])),
            "boolean": Style.parse(variables.get("syntax-json-boolean", variables["text-success"])),
            "json.null": Style.parse(variables.get("syntax-json-null", variables["text-warning"])),
            "json.label": Style.parse(variables.get("syntax-json-key", variables["text-primary"])),
        }
        ...
        return TextAreaTheme(
            ...
            cursor_line_gutter_style=Style.parse(variables.get("text-area-cursor-line-gutter"))
            if "text-area-cursor-line-gutter" in variables
            else None,
            bracket_matching_style=Style.parse(variables.get("text-area-matched-bracket"))
...
def load_user_themes(directory: Path | None = None) -> UserThemeLoadResult:
    """Load user themes from the theme directory.

    Args:
        directory: Path to theme directory. If None, returns empty result.

    Returns:
        A UserThemeLoadResult with loaded themes and any failures.
    """
